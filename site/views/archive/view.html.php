 ...
 * @since  1.5
 */
class BlogViewArchive extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		foreach ($items as $item)
		{
			...
			$dispatcher->trigger('onContentPrepare', array ('com_blog.archive', &$item, &$item->params, 0));

			// Old plugins: Use processed text as introtext
			$item->introtext = $item->text;

			$results = $dispatcher->trigger('onContentAfterTitle', array('com_blog.archive', &$item, &$item->params, 0));
			$item->event->afterDisplayTitle = trim(implode("\n", $results));

			$results = $dispatcher->trigger('onContentBeforeDisplay', array('com_blog.archive', &$item, &$item->params, 0));
			$item->event->beforeDisplayContent = trim(implode("\n", $results));

			$results = $dispatcher->trigger('onContentAfterDisplay', array('com_blog.archive', &$item, &$item->params, 0));
