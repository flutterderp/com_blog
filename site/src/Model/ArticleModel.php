...
class ArticleModel extends ItemModel
{
    ...
    public function getItem($pk = null)
    {
        ...
        if (!isset($this->_item[$pk])) {
            try {
                ...
                $query->select(
                    $this->getState(
                        ...
                        [
                            ...
                            $db->quoteName('a.gallery'),
                            $db->quoteName('a.video_uri'),
                            $db->quoteName('a.toggle_sources_type'),
                            $db->quoteName('a.sources'),
                            $db->quoteName('a.sources_blob'),
                            $db->quoteName('a.extra1'),
                            $db->quoteName('a.extra2'),
                            $db->quoteName('a.extra3'),
                ...
                if (
                    !$user->authorise('core.edit.state', 'com_blog.article.' . $pk)
                    && !$user->authorise('core.edit', 'com_blog.article.' . $pk)
                ) {
    ...
    public function hit($pk = 0)
    {
        ...
        if ($hitcount) {
            ...
            $table = Table::getInstance('ArticleTable', '\\Joomla\\Component\\Blog\\Administrator\\Table\\');
