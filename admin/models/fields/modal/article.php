	...
	protected function getInput()
	{
		...
		if ($allowClear && !$scriptClear)
		{
			...
			$script[] = '		document.getElementById(id + "_name").value = "' .
				htmlspecialchars(JText::_('COM_BLOG_SELECT_AN_ARTICLE', true), ENT_COMPAT, 'UTF-8') . '";';
		...
		$html = array();
		$link = 'index.php?option=com_blog&amp;view=articles&amp;layout=modal&amp;tmpl=component&amp;function=jSelectArticle_' . $this->id;
		...
		if (empty($title))
		{
			$title = JText::_('COM_BLOG_SELECT_AN_ARTICLE');
		}
		...
		$html[] = '<a href="#modalArticle' . $this->id . '" class="btn hasTooltip" role="button"  data-toggle="modal" title="'
			. JHtml::tooltipText('COM_BLOG_CHANGE_ARTICLE') . '">'
		...
		if ($allowEdit)
		{
			$html[] = '<a class="btn hasTooltip' . ($value ? '' : ' hidden')
				. '" href="index.php?option=com_blog&layout=modal&tmpl=component&task=article.edit&id=' . $value . '" target="_blank" title="'
				. JHtml::tooltipText('COM_BLOG_EDIT_ARTICLE') . '" ><span class="icon-edit"></span>' . JText::_('JACTION_EDIT') . '</a>';
		}
		...
		$html[] = JHtml::_(
			...
			array(
				'url' => $url,
				'title' => JText::_('COM_BLOG_CHANGE_ARTICLE'),
				'width' => '800px',
				'height' => '300px',
