...
class JFormFieldModal_Article extends JFormField
{
	...
	protected function getInput()
	{
		...
		$linkArticles = 'index.php?option=com_blog&amp;view=articles&amp;layout=modal&amp;tmpl=component&amp;' . JSession::getFormToken() . '=1';
		$linkArticle  = 'index.php?option=com_blog&amp;view=article&amp;layout=modal&amp;tmpl=component&amp;' . JSession::getFormToken() . '=1';

		if (isset($this->element['language']))
		{
			$linkArticles .= '&amp;forcedLanguage=' . $this->element['language'];
			$linkArticle  .= '&amp;forcedLanguage=' . $this->element['language'];
			$modalTitle    = JText::_('COM_BLOG_CHANGE_ARTICLE') . ' &#8212; ' . $this->element['label'];
		}
		else
		{
			$modalTitle    = JText::_('COM_BLOG_CHANGE_ARTICLE');
		}
		...
		$title = empty($title) ? JText::_('COM_BLOG_SELECT_AN_ARTICLE') : htmlspecialchars($title, ENT_QUOTES, 'UTF-8');
		...
		if ($allowSelect)
		{
			$html .= '<button'
				...
				. ' title="' . JHtml::tooltipText('COM_BLOG_CHANGE_ARTICLE') . '">'
		...
		if ($allowNew)
		{
			$html .= '<button'
				...
				. ' title="' . JHtml::tooltipText('COM_BLOG_NEW_ARTICLE') . '">'
		...
		if ($allowEdit)
		{
			$html .= '<button'
				...
				. ' title="' . JHtml::tooltipText('COM_BLOG_EDIT_ARTICLE') . '">'
		...
		if ($allowNew)
		{
			$html .= JHtml::_(
				...
				'ModalNew' . $modalId,
				array(
					'title'       => JText::_('COM_BLOG_NEW_ARTICLE'),
					'backdrop'    => 'static',
					'keyboard'    => false,
					'closeButton' => false,
					'url'         => $urlNew,
		...
		if ($allowEdit)
		{
			$html .= JHtml::_(
				...
				'ModalEdit' . $modalId,
				array(
					'title'       => JText::_('COM_BLOG_EDIT_ARTICLE'),
					'backdrop'    => 'static',
					'keyboard'    => false,
					'closeButton' => false,
					'url'         => $urlEdit,
