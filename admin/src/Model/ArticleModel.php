...
class ArticleModel extends AdminModel implements WorkflowModelInterface/* , VersionableModelInterface */
{
    ...
    /**
     * The type alias for this content type (for example, 'com_blog.article').
     *
     * @var    string
     * @since  3.2
     */
    ...
    public function __construct($config = [], ?MVCFactoryInterface $factory = null, ?FormFactoryInterface $formFactory = null)
    {
        ...
        $this->setUpWorkflow('com_blog.article');
    }
    ...
    {
        // Check if the article was featured and update the #__blog_frontpage table
        if ($table->featured == 1) {
            $db    = $this->getDatabase();
        ...
        $oldItem->load($oldId);
        $fields = FieldsHelper::getFields('com_blog.article', $oldItem, true);
        ...
        Factory::getApplication()->triggerEvent('onContentAfterSave', ['com_blog.article', &$this->table, false, $fieldsData]);
    }
    ...
    protected function batchMove($value, $pks, $contexts)
    {
        ...
        foreach ($pks as $pk) {
            ...
            $fields = FieldsHelper::getFields('com_blog.article', $this->table, true);
    ...
    protected function canDelete($record)
    {
        ...
        return $this->getCurrentUser()->authorise('core.delete', 'com_blog.article.' . (int) $record->id);
    }
    ...
    protected function canEditState($record)
    {
        ...
        if (!empty($record->id)) {
            return $user->authorise('core.edit.state', 'com_blog.article.' . (int) $record->id);
        }
    ...
    public function getItem($pk = null)
    {
        if ($item = parent::getItem($pk)) {
            ...
            // Convert the gallery field to an array.
            $registry      = new Registry($item->gallery);
            $item->gallery = $registry->toArray();

            // Convert the sources field to an array.
            ...
            if (!empty($item->id)) {
                ...
                $item->tags->getTagIds($item->id, 'com_blog.article');

                if (!is_null($item->secondary_categories)) {
                    $item->secondary_categories = explode(',', $item->secondary_categories);
                }
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        // Get the form.
        $form = $this->loadForm('com_blog.article', 'article', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    protected function loadFormData()
    {
        ...
        $app  = Factory::getApplication();
        $data = $app->getUserState('com_blog.edit.article.data', []);
        ...
        if (empty($data)) {
            ...
            if ($this->getState('article.id') == 0) {
                $filters     = (array) $app->getUserState('com_blog.articles.filter');
        ...
        $this->preprocessData('com_blog.article', $data);
    ...
    public function save($data)
    {
        ...
        if (isset($data['gallery']) && \is_array($data['gallery'])) {
            $registry = new Registry($data['gallery']);

            $data['gallery'] = (string) $registry;
        }

        if (isset($data['sources']) && \is_array($data['sources'])) {
            $registry = new Registry($data['sources']);

            $data['sources'] = (string) $registry;
        }

        if (isset($data['secondary_categories']) && \is_array($data['secondary_categories'])) {
            $data['secondary_categories'] = implode(',', $data['secondary_categories']);
        }
        ...
        if (is_numeric($data['catid']) && $data['catid']) {
            $createCategory = !CategoriesHelper::validateCategoryId($data['catid'], 'com_blog');
        }
        ...
        if (parent::save($data)) {
            ...
            if (isset($data['featured']) && !$this->bootComponent('com_blog')->isFunctionalityUsed('core.featured', 'com_blog.article')) {
                if (
                    !$this->featured(
    ...
    /**
     * Delete #__blog_frontpage items if the deleted articles was featured
     *
    ...
    public function delete(&$pks)
    {
        ...
        if ($return) {
            // Now check to see if this articles was featured if so delete it from the #__blog_frontpage table
            $db    = $this->getDatabase();
            $query = $db->getQuery(true)
                ->delete($db->quoteName('#__blog_frontpage'))
                ->whereIn($db->quoteName('content_id'), $pks);
